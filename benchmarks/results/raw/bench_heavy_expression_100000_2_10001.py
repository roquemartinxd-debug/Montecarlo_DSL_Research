...
def _simulate_batch(batch_index, iterations):
    ...
    _mc_var_0 = rng.normal(loc=0.0, scale=1.0, size=iterations).astype(np.float64, copy=False)
    _mc_var_1 = rng.uniform(low=-3.0, high=3.0, size=iterations).astype(np.float64, copy=False)

    branch_values = []
    branch_raw_values = []
    with np.errstate(all="ignore"):
        _raw_0 = _coerce_result_array((((((((np.sqrt((((np.power((_mc_var_0), (2.0))) + (np.power((_mc_var_1), (2.0))))))) + (((((_mc_var_0) * (_mc_var_1))) / (2.0))))) + (((3.0) * (_mc_var_0))))) - (((2.0) * (_mc_var_1))))), iterations)
