...
def _simulate_batch(batch_index, iterations):
    ...
    _mc_var_0 = rng.choice(np.array([10.0, 20.0, 30.0], dtype=np.float64), size=iterations, p=np.array([0.5, 0.3, 0.2], dtype=np.float64))

    branch_values = []
    branch_raw_values = []
    with np.errstate(all="ignore"):
        _raw_0 = _coerce_result_array((_mc_var_0), iterations)
